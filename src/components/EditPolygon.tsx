...
interface EditPolygonProps {
  activePolygonData: Polygon;
  updatePolygon: (e: any) => void;
  setActivePoly: React.Dispatch<React.SetStateAction<number | undefined>>;
  copyJSON: (e: any) => void;
  copySVGTag: (points: Point2D[], name: string) => Promise<void>;
  copyCSSClipPath: (points: Point2D[], name: string) => Promise<void>;
  handleRemovePolygon: (e: number) => void;
}

export default function EditPolygon({activePolygonData,setActivePoly,updatePolygon,copyJSON,copySVGTag,copyCSSClipPath,handleRemovePolygon}:EditPolygonProps) {
  return (
    <div className="h-screen bg-[#1b1b1c] flex flex-col z-20 overflow-hidden border-l border-[#1b1b1c] w-80">
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="text-[#e5e2e1]/80">
            {activePolygonData.points.map((p: Point2D, index: number) => (
              <tr className="border-b border-[#414755]/30 hover:bg-[#2a2a2a] transition-colors">
      ...
      <div className="p-3 border-t border-[#414755]">
        ...
        <div className="flex gap-2">
          <button onClick={() => copySVGTag(activePolygonData.points, activePolygonData.name)} className="cursor-pointer flex-1 py-2 bg-[#4ae176] hover:opacity-90 text-[#003915] rounded flex items-center justify-center gap-2  transition-all">
            <span className="font-semibold leading-4 tracking-wider text-[11px]">SVG Tag</span>
          </button>
          <button onClick={() => copyCSSClipPath(activePolygonData.points, activePolygonData.name)} className="cursor-pointer flex-1 py-2 bg-[#4ae176] hover:opacity-90 text-[#003915] rounded flex items-center justify-center gap-2  transition-all">
      ...
      <div className="p-3 border-t border-[#414755] flex gap-2">
        <button onClick={() => handleRemovePolygon(activePolygonData.id)} className="cursor-pointer flex-1 py-2 bg-[#ffdad6] text-[#690005] border border-[#ffb4ab]/50 rounded flex items-center justify-center gap-2 hover:bg-[#ffb4ab] transition-all hover:text-[#690005]">
